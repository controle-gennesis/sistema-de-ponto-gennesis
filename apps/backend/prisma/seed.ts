...
async function main() {
  ...
  try {
    ...
    // Verificar se já existe um funcionário com employeeId 'DIR001'
    ...
    if (existingEmployee) {
      ...
      director = existingEmployee.user;
      employee = existingEmployee;
      console.log('ℹ️  Funcionário diretor já existe, usando dados existentes');
    } else {
      // Criar novo usuário diretor
      director = await prisma.user.upsert({
    ...
    // Se o erro for de constraint única, apenas avisar mas não falhar
    if (error?.code === 'P2002') {
      console.log('ℹ️  Funcionário diretor já existe no banco de dados, pulando criação');
    } else {
      console.error('⚠️  Erro ao criar funcionário diretor:', error);
      // Continuar mesmo se houver erro
    }
  }

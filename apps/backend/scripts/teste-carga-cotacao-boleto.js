/**
 * Teste de carga k6 — Mapa de Cotação → geração de OC (BOLETO_30)
 *
 * Variante de teste-carga-cotacao.js: gera OCs com paymentType=BOLETO sem
 * boletoAttachmentUrl (boleto anexado depois na fase Pagamento).
 *
 * Pré-requisitos:
 ...
 *   k6 run -e VUS=5 -e ITERATIONS=20 scripts/teste-carga-cotacao-boleto.js
 *
 * Variáveis opcionais:
...
const PAYMENT_CONDITION = __ENV.PAYMENT_CONDITION || 'BOLETO_30';
    ...
    paymentBySupplier: [
      {
        ...
        observations: 'OC gerada por teste de carga boleto',
      },
    ],
  };
...
export function setup() {
  ...
  console.log(
    `setup — APPROVED=${approved.length} | ITERATIONS=${ITERATIONS} | ` +
      ...
      `payment=${PAYMENT_CONDITION} | user=${USER_EMAIL}`,
  );
...
export default function (data) {
  ...
  check(null, {
    'pipeline cotação boleto completo': () => true,
  });

/**
 * Teste de carga k6 — Pagamento AVISTA de OC (APPROVED → FINALIZED)
 *
 * Pré-requisitos:
 *   1. Backend rodando (padrão: http://localhost:5000)
 *   2. teste1@loadtest.com autenticável (criador das OCs)
 *   3. OCs com status=APPROVED e paymentType=AVISTA
 *      (ex.: teste-carga-cotacao.js + teste-carga-aprovacao-oc.js)
 *   4. k6 instalado
 *
 * Com 20 OCs APPROVED (AVISTA):
 *   k6 run -e VUS=5 -e ITERATIONS=20 scripts/teste-carga-pagamento-oc.js
 *
 * Uma OC distinta por iteração (shared-iterations) para evitar concorrência na mesma OC.
 *
 * Variáveis opcionais:
...
function fetchApprovedAvistaOrders(token) {
  ...
  return list
    .filter((o) => o?.id && String(o.paymentType || '').toUpperCase() === 'AVISTA')
    .map((o) => ({
      id: o.id,
...
function runPaymentFlow(token, order, paymentMonth, paymentYear) {
  ...
  if (!patchStatus(token, id, 'PENDING_PROOF_VALIDATION', 'status_proof_validation', 'PENDING_PROOF_VALIDATION')) {
    return false;
  }

  if (!patchStatus(token, id, 'PENDING_NF_ATTACHMENT', 'status_nf_attachment', 'PENDING_NF_ATTACHMENT')) {
    return false;
  }
...
export function setup() {
  ...
  const approved = fetchApprovedAvistaOrders(session.token);
  const now = new Date();
  ...
  if (approved.length === 0) {
    throw new Error(
      'Nenhuma OC APPROVED com paymentType=AVISTA. Rode cotação + aprovação antes.',
    );
  }

  if (approved.length < ITERATIONS) {
    throw new Error(
      `Só há ${approved.length} OC(s) APPROVED+AVISTA, mas ITERATIONS=${ITERATIONS}. ` +
...
export default function (data) {
  ...
  check(null, {
    'iteração finalizou OC de fato': () => ok,
  });

  if (!ok) {
    fail(
      `VU ${__VU}: falha no pagamento AVISTA da OC ${order.orderNumber || order.id} (iterationInTest=${idx})`,
    );
  }

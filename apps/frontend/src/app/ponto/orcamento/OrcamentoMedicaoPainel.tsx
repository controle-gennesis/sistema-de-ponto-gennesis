...
export function OrcamentoMedicaoPainel({
  ...
  const renderLinhaCampos = (ln: LinhaMedicao, idx: number) => {
    ...
    const fieldLabel = 'text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 block mb-1.5';
    /** Valores calculados — uma cor por tipo (área / volume / quantidade na linha) */
    const fieldLabelArea =
      'text-[10px] font-semibold uppercase tracking-wider text-sky-700 dark:text-sky-400 block mb-1.5';
    const fieldLabelVolume =
      'text-[10px] font-semibold uppercase tracking-wider text-red-700 dark:text-red-400 block mb-1.5';
    const fieldLabelSubtotal =
      'text-[10px] font-semibold uppercase tracking-wider text-emerald-800 dark:text-emerald-400 block mb-1.5';
    /** Carga manual: rótulos de dados vindos das demolições (não editáveis) */
    const fieldLabelOrigemCarga =
      'text-[10px] font-semibold uppercase tracking-wider text-slate-500/90 dark:text-slate-500 block mb-1.5';
    const inputBase =
    ...
    const caixaArea =
      'h-9 px-2.5 rounded-md border border-sky-400/50 dark:border-sky-600/50 bg-sky-50 dark:bg-sky-950/45 text-sm font-semibold tabular-nums text-sky-950 dark:text-sky-100 flex items-center justify-end cursor-default select-none';
    const caixaVolume =
      'h-9 px-2.5 rounded-md border border-red-400/50 dark:border-red-600/50 bg-red-50 dark:bg-red-950/45 text-sm font-semibold tabular-nums text-red-950 dark:text-red-100 flex items-center justify-end cursor-default select-none';
    const caixaSubtotal =
      'h-9 px-2.5 rounded-md border border-emerald-400/50 dark:border-emerald-600/50 bg-emerald-50 dark:bg-emerald-950/50 text-sm font-semibold tabular-nums text-emerald-950 dark:text-emerald-100 flex items-center justify-end cursor-default select-none';

    return (
      <div key={idx} className="flex flex-wrap items-end gap-x-3 gap-y-3">
        <div className="flex-1 min-w-[min(100%,220px)]">
          <label className={bloquearDescricao ? fieldLabelOrigemCarga : fieldLabel}>Descrição</label>
          <input
            type="text"
        ...
        {mostrarC && (
          <div className="w-[88px] shrink-0">
            <label className={ehCargaEntulho && !podeEditarCNaCarga ? fieldLabelOrigemCarga : fieldLabel}>C (m)</label>
            <input
              type="text"
              inputMode="decimal"
              placeholder="0"
              ...
              className={
                ehCargaEntulho && !podeEditarCNaCarga
                  ? `${inputBloqueadoCarga} text-right`
                  : `${inputBase} text-right bg-white dark:bg-gray-800/90`
              }
            />
          </div>
        )}
        {mostrarL && (
          <div className="w-[88px] shrink-0">
            <label className={ehCargaEntulho && !podeEditarLNaCarga ? fieldLabelOrigemCarga : fieldLabel}>L (m)</label>
            <input
              type="text"
              inputMode="decimal"
              placeholder="0"
        ...
        {mostrarH && (
          <div className="w-[88px] shrink-0">
            <label className={ehCargaEntulho && !podeEditarHNaCarga ? fieldLabelOrigemCarga : fieldLabel}>H (m)</label>
            <input
              type="text"
              inputMode="decimal"
              placeholder="0"
        ...
        {mostrarN && (
          <div className="w-[68px] shrink-0">
            <label className={bloquearN ? fieldLabelOrigemCarga : fieldLabel}>N</label>
            <input
              type="text"
              inputMode="decimal"
              placeholder="1"
              ...
              readOnly={bloquearN}
              className={bloquearN ? `${inputBloqueadoCarga} text-right` : `${inputBase} text-right bg-white dark:bg-gray-800/90`}
            />
          </div>
        )}
        <div className="w-[84px] shrink-0">
          <label className={fieldLabel} title={ehCargaEntulho ? 'Fator de empolamento — único campo editável nesta linha' : 'Fator de empolamento / perdas'}>
            %
          </label>
          <input
            type="text"
            ...
            className={`${inputBase} text-right bg-white dark:bg-gray-800/90`}
          />
        </div>
        {mostrarA && (
          <div className="w-[88px] shrink-0">
            <label className={temDimensoes ? fieldLabelArea : ehCargaEntulho ? fieldLabelOrigemCarga : fieldLabel}>A</label>
            {!temDimensoes ? (
              <input
                type="text"
                inputMode="decimal"
                ...
                className={ehCargaEntulho ? `${inputBloqueadoCarga} text-right` : `${inputBase} text-right bg-white dark:bg-gray-800/90`}
              />
            ) : (
              <div
                className={caixaArea}
        ...
        {mostrarV && (
          <div className="w-[88px] shrink-0">
            <label className={temDimensoes ? fieldLabelVolume : ehCargaEntulho ? fieldLabelOrigemCarga : fieldLabel}>V</label>
            {!temDimensoes ? (
              <input
                type="text"
                inputMode="decimal"
        ...
        <div className="w-[104px] shrink-0">
          <label className={fieldLabelSubtotal}>Subtotal</label>
          <div
    ...
    return (
      <div className="space-y-3">
        {dim.linhas.map((ln, idx) => (
          <div key={idx} className={linhaCard}>
